...
- Use `LOAD:SETVOLT\t<voltage>` to adjust the set-point, where `<voltage>` is the required voltage in \[V\]
...
- Use `LOAD:SETCURR\t<current>` to adjust the set-point, where `<current>` is the required current in \[A\]
...
- To measure an IV curve, following program flow steps should be taken:
    ...
    - OPET replies with `IV:MEAS\t<XXX>`, where `<XXX>` is the estimated IV curve measurement time in ms
...
- OPET devices do not yet recognise multiple commands sent at the same time
...
| `IV:VOC:MULT`      | V<sub>oc</sub> measurement multiplier of IV control end voltage  |
...
- Each bit has following meaning, false is inactive, true is active:
    ...
    - Bit 4: Overcurrent bypass active
...
- The internal reference voltage offset `Voff` is a reference signal that should always be the same
    ...
    - this reading could be used for advanced drift corrections on readings and voltage control, but this has not yet been implemented
...
- Following values are valid:
    ...
    - `1` V<sub>oc</sub> -- open circuit load
    - `2` I<sub>sc</sub> -- short circuit load
    - `3` V<sub>set</sub> -- constant voltage load at given voltage set-point see below
    - `4` C<sub>set</sub> -- constant current load at given current set-point see below
...
- Voltage set-point value in volts can be anything, but must be positive, if the set-point is too high and cannot be reached, the system will operate at V<sub>oc</sub>
...
- Current set-point value in amperes can be anything, but must be positive, if the set-point is too high and cannot be reached, the system will operate at I<sub>sc</sub>
...
- Currently, this value is only used to calculate the time delays for transient measurements
...
- The exact time *t* the counter takes is calculated as follows, where *C<sub>val</sub>* is the loop timer value, *64* is the clock divider and *F<sub>MCU</sub>* is the frequency of the microcontroller:
...
### Autorange mode range enable/disable state bytes
...
- following conversion formula is used, where x is the ADC reading: $V_{b} = A0 + x*A1$
...
- NOTE: In the current version, any value below \~−4°C cannot be reached as the NTC resistance is too high and overflowing the ADC input
...
- NOTE: as detailed above, in the current version any value below \~−4°C cannot be reached as the NTC resistance is too high and overflowing the ADC input
...
#### Cosine sweep maximum phase angle
- Register ID: `142`
...
- Value Range: `single floating point`
- As further detailed in [V<sub>oc</sub> overshoot control multiplier](#voc-overshoot-control-multiplier), this multiplier is applied to the measured V<sub>oc</sub> before IV measurements and sets the IV curve endpoint control voltage at the voltage control DAC
...
- It essentially controls the step size the MPPT tracer takes to reach the P<sub>mp</sub> at the start
- Using a too high value here may cause oscillations around the maximum power point
- Using a too low value here means it will take a long time for the P<sub>mp</sub> to be reached or it may get stuck due to signal noise
...
- This controls the step size applied after the MPPT has reached P<sub>mp</sub> and is "hovering" around this point
...
- Value Range: `single floating point`
- The step size increase multiplier determines the increase in step size per control cycle when the MPPT tracer has lost its P<sub>mp</sub> tracking, it essentially controls how fast it will get back to maximum step size
- The percentage increase in step size (here standard is 20%) should be lower than the percentage reduction in step size to assure optimal MPPT control

#### Step size reduction multiplier 
- Register ID: `153`
- Value: default `0.6`, standard `0.6`
- Value Range: `single floating point`
- The step size reduction multiplier determines the reduction in step size per control cycle when the MPPT tracer has found the P<sub>mp</sub> point, it essentially controls how fast it will reduce to minimum step size
- The percentage reduction in step size (here standard is 40%) should be larger than the percentage increase in step size and less than 50% to assure optimal MPPT control
...
- A large value results in the MPPT wandering around the P<sub>mp</sub> point continuously
- A too low value may result in the tracker not tracking around the exact P<sub>mp</sub>.
...
- The percentage increase in step size (here standard is 20%) should be lower than the percentage reduction in step size to assure optimal control
...
- The percentage reduction in step size (here standard is 40%) should be larger than the percentage increase in step size and less than 50% to assure optimal control
...
1. Check the deviation between old and new scale factors
    ...
    - Calculate the percentage deviation to the scale of each
...
- The validation of the calibration is pretty much the same as the calibration with the following exceptions:
    ...
    - Instead of reading the original calibration factors, use ideal factors as *Scale = 1* and *Offset = 0* and calculate the percentage deviations to scale with those.
